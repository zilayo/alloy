...
use alloy_rpc_types_eth::{state::StateOverride, TransactionInputKind};
...
/// Basic version of [crate::MulticallBuilder] to allow using multicall within type constraints.
#[derive(Debug)]
    ...
    pub async fn aggregate3(&self) -> Result<Vec<Result<Vec<DynSolValue>, Failure>>> {
        let calls = self
            ...
            .map(|c| {
                let encoded_call = c
                    .decoder
                    .abi_encode_input(&c.params)
                    .map_err(MulticallError::DynamicEncodeError)?;
            ...
            let decoded_call_result = match result.success {
                true => {
                    let decoded = self.calls[idx]
                        ...
                        .map_err(MulticallError::DynamicDecodeError)?;
                    Ok(decoded)
                }
                false => Err(Failure { idx, return_data: result.returnData.clone() }),
            };
...
impl DynCallItem {
    ...
    pub const fn new(target: Address, params: Vec<DynSolValue>, function: Function) -> Self {
        Self { target, params, allow_failure: false, value: U256::ZERO, decoder: function }
    }
